...
class LineFollower(object):
    ...
    def callback_wall(self, data):
        global right_min, left_min, front_min, front1, front_cone1, right1, ind
        # left = data.ranges[90]
        # lase = data.ranges[1:359]
        # print(lase)
        right1 = data.ranges[270]
        front1 = data.ranges[0]
        p = []
        p = data.ranges

        front_cone1 = []
        for j in range(340,360):
            if p[j] ==0: front_cone1.append(999)
            else: front_cone1.append(p[j])
        for k in range(0,21):
            if p[k]==0: front_cone1.append(999)
            else: front_cone1.append(p[k])
        front1 = min(front_cone1)
        ind = p.index(front1)
    ...
    def wall_avoid(self):
        global right_min, left_min, front_min,d, det_flag, front1, ind
